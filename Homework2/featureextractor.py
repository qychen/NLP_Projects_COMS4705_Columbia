...
#                    result.append('BUF_2_CPOSTAG_' + token['ctag'])
        ...
        if buffer and stack:
            buf_addr = tokens[buffer[0]]['address']
            stk_addr = tokens[stack[-1]]['address']

#            sum = 0
#            for i in range(stk_addr+1,buf_addr):
#                if tokens[i]['ctag']=="VV" or tokens[i]['ctag']=="V" or tokens[i]['ctag']=="VERB":
#                    sum += 1
#            result.append('VDIS' + str(sum))
            result.append('DIS_' + str(tokens[buffer[0]]['address']-tokens[stack[-1]]['address'])) 
            result.append('STK_NCHILD_' + str(len(tokens[stack[-1]]['deps'])))
            result.append('BUF_NCHILD_' + str(len(tokens[buffer[0]]['deps'])))
