...
def extract_features(data, lang, feas_set, wordsbag):
    ...
    for ins in data:
        ...
        #a. surrounding words
        if len(left_context) >= 2:
            feas["w-2"] = stemmer.stem(left_context[-2])
        if len(left_context) >= 1:
            feas["w-1"] = stemmer.stem(left_context[-1])
        feas["w0"] = ins[2]
        if len(right_context) >= 2:
            feas["w2"] = stemmer.stem(right_context[1])
        if len(right_context) >= 1:
            feas["w1"] = stemmer.stem(right_context[0])
        #if lang == "English":
        #    if len(left_context) >= 3:
        ...
        #uni-gram
        if lang != "English":
            if len(left_context) > unigram_size:
                left = left_context[-1*unigram_size:]
            else:
                left = left_context
            if len(right_context) > unigram_size:
                right = right_context[:unigram_size]
            else:
                right = right_context
        ...
        if len(left_context) >= 1:
            if lang == "English":
                ...
                synset = wn.synsets(stemmer.stem(left_context[-1]), lang = 'spa')
            else:
                synset = wn.synsets(stemmer.stem(left_context[-1]), lang = 'cat')
        ...
        if len(right_context) >= 1:
            if lang == "English":
                ...
                synset = wn.synsets(stemmer.stem(right_context[0]), lang = 'spa')
            else:
                synset = wn.synsets(stemmer.stem(right_context[0]), lang = 'cat')
            for syn in synset:
                hypers = syn.hypernyms()
                for hyper in hypers:
                    name = hyper.name().split(".")[0]
        ...
        '''
        if len(left_context) >= 2:
            synset = wn.synsets(stemmer.stem(left_context[-2]))
            for syn in synset:
                hypers = syn.hypernyms()
                for hyper in hypers:
                    name = hyper.name().split(".")[0]
                    if name not in left_hypers:
                        left_hypers.append(name)
        if len(right_context) >= 2:
            synset = wn.synsets(stemmer.stem(right_context[1]))
            for syn in synset:
                hypers = syn.hypernyms()
                for hyper in hypers:
                    name = hyper.name().split(".")[0]
...
def feature_selection(X_train,X_test,y_train):
    ...
    selector = SelectKBest(chi2, k=len(X_train_trans[0])*0.87)
